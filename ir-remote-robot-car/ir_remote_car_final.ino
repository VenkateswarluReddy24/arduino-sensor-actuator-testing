...
void RunMotors(unsigned long runTime)
{
  ...
  while (micros() - startTime < (runTime * 1000UL))
  {
    ...
    unsigned long onTime =
      ((unsigned long)pwmValue * PWM_PERIOD) / 255UL;
...
void Rotate360()
{
  ...
  // Use the currently selected speed
...
void loop()
{
  if (IrReceiver.decode())
  {
    ...
    // Prepare receiver for next frame
    IrReceiver.resume();

    // Ignore held-button repeat frames
    if (repeat)
    {
    ...
    else if (command == CMD_4)
    {
      SetSpeed(4);
    }

    else if (command == CMD_5)
    {
      SetSpeed(5);
    }

    else if (command == CMD_6)
    {
      SetSpeed(6);
    }

    else if (command == CMD_7)
    {
      SetSpeed(7);
    }
