/*
  ...
  - Maintain original movement behavior
...
// ---------------- COMMAND TIMEOUT ----------------
// Robot will automatically stop if no movement
// command is received for more than 3 seconds.

const unsigned long COMMAND_TIMEOUT = 3000;

// Stores the time of the last movement command
...
void setup() {
  ...
  Serial.println("⏱ Command Timeout: 3 Seconds");
}
...
void loop() {
  ...
  if (millis() - lastCommandTime > COMMAND_TIMEOUT) {
    ...
    // Reset timer so stopRobot() is not repeatedly
    // called every loop
    lastCommandTime = millis();
  }
}
